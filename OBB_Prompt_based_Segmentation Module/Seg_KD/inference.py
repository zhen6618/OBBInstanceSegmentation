...
def draw_mask(image, bboxes, pred_mask, save_path, draw_bboxes):
...
def main(cfg: Box) -> None:
    ...
    'pred'
    image_path = 'datasets_debug/val_images/6.png'
    rbox_path = image_path.replace('val_images', 'val_box_labels').replace('png', 'txt')

    'ge_mask'
    # gt_masks = [np.zeros((1024, 1024), dtype=np.uint8)]

    annotation_file=  "datasets_industry/annotations/val_sam.json"
    ...
    for pred_mask, gt_mask in zip(pred_masks, gt_masks):
        ...
        draw_mask(images, bboxes, pred_mask, save_path='demo_pred_mask.png', draw_bboxes=True)
        draw_mask(images, bboxes, gt_mask, save_path='demo_gt_mask.png', draw_bboxes=False)
