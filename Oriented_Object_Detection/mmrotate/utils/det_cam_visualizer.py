...
def reshape_transform(feats, max_shape=(64, 64), is_need_grad=False):  # 全部变换到最小size
...
class DetCAMVisualizer:
    ...
    def show_cam(self,
        ...
        # renormalized_cam = 1 - renormalized_cam  # inverse heatmap
    ...
    def _draw_boxes(self, boxes, labels, image):
        for i, box in enumerate(boxes):
            ...
            x, y, w, h, angle = box[0], box[1], box[2], box[3], box[4]
            cos_, sin_ = math.cos(angle), math.sin(angle)

            x1_h, y1_h = x - w / 2, y + h / 2
            x2_h, y2_h = x + w / 2, y + h / 2
            x3_h, y3_h = x + w / 2, y - h / 2
            x4_h, y4_h = x - w / 2, y - h / 2

            x1_r, y1_r = x + (x1_h - x) * cos_ - (y1_h - y) * sin_, y + (x1_h - x) * sin_ + (y1_h - y) * cos_
            x2_r, y2_r = x + (x2_h - x) * cos_ - (y2_h - y) * sin_, y + (x2_h - x) * sin_ + (y2_h - y) * cos_
            x3_r, y3_r = x + (x3_h - x) * cos_ - (y3_h - y) * sin_, y + (x3_h - x) * sin_ + (y3_h - y) * cos_
            x4_r, y4_r = x + (x4_h - x) * cos_ - (y4_h - y) * sin_, y + (x4_h - x) * sin_ + (y4_h - y) * cos_

            cv2.line(image, (int(x1_r), int(y1_r)), (int(x2_r), int(y2_r)), color, thickness=2)
            cv2.line(image, (int(x2_r), int(y2_r)), (int(x3_r), int(y3_r)), color, thickness=2)
            cv2.line(image, (int(x3_r), int(y3_r)), (int(x4_r), int(y4_r)), color, thickness=2)
            cv2.line(image, (int(x4_r), int(y4_r)), (int(x1_r), int(y1_r)), color, thickness=2)
    ...
    def __call__(self, results):
        ...
        if 'loss_cls' in results:
            ...
            for loss_key, loss_value in results.items():
                ...
                # if 'loss' not in loss_key:  # bbox + cls
                if 'bbox' not in loss_key:  # only bbox
                # if 'cls' not in loss_key:  # only cls
            ...
            for focal_box, focal_label in zip(self.focal_bboxes, self.focal_labels):
                ...
                if ious[0, index] > self.match_iou_thr and pred_labels[
                    ...
                    # score = ious[0, index] + pred_bboxes[..., 4][index]  # bbox + cls_score
                    score = ious[0, index]  # only bbox
                    output = output + score
